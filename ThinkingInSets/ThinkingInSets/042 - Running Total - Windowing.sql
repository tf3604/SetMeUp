select oh.OrderId, oh.OrderDate, oh.CustomerId,
	row_number() over (order by od.OrderDetailId) OrderLineNbr,
	od.ProductId, od.Quantity, od.UnitPrice, od.Quantity * od.UnitPrice Amount,
	sum(od.Quantity * od.UnitPrice) over (order by od.OrderDetailId rows unbounded preceding) RunningTotal
